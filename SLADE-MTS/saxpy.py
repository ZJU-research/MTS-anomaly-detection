...
class SAX(object):
    ...
    def __init__(self, wordSize = 8, alphabetSize = 7, epsilon = 1e-6, upperBound = 100, lowerBound = -100):
    ...
    def to_letter_rep_ori(self, x):
        """
        Function takes a series of data, x, and transforms it to a string representation(fix height)
        """
        (paaX, indices) = self.to_PAA(x)
        return (self.alphabetize_ori(paaX), indices)
    ...
    def to_PAA(self, x):
        ...
        """
        n = len(x)
        stepFloat = n/float(self.wordSize)  #every step size in float
        step = int(math.ceil(stepFloat)) #float size in ceil
        frameStart = 0
        approximation = []
        indices = []
        i = 0
    ...
    def alphabetize_ori(self,paaX):
        """
        Converts the Piecewise Aggregate Approximation of x to a series of letters.(fixed height)
        """
        alphabetizedX = ''
        for i in range(0, len(paaX)):
            for j in range(0, self.alphabetSize):
                if np.isnan(paaX[i]):
                    alphabetizedX += '-'
                    break
                currentBound = self.lowerBound + (self.upperBound - self.lowerBound) / self.alphabetSize * (j + 1)
                if paaX[i] < currentBound:
                    alphabetizedX += chr(self.aOffset + j)
                    #print(str(paaX[i])+' '+str(currentBound)+' '+alphabetizedX)
                    break
