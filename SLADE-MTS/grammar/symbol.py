...
class Symbol(object):
    ...
    def print_rule_expansion(self, _, output_array, line_length, rules):
        """docstring for print_rule_expansion"""
        output_array.append(self.print_terminal())
        #print(self.print_terminal())
        segment = self.grammar.segments[self.grammar.count]
        # print(segment.content())
        # print('count:'+str(self.grammar.count))
        # print(id(segment))
        # print(rule.unique_number)
        # print('----')
        segment.allCoveredRules = rules[:]
        segment.setRule(rules[-1])
        self.grammar.count += 1
    ...
    def value(self):
        """docstring for value"""
        return (self.rule.unique_number if self.rule else self.terminal.content())
...
class Terminal(Symbol):
    ...
    string_value = value  #Function Equaltion
...
class NonTerminal(Symbol):
    ...
    def print_rule(self, rule_set, output_array, line_length):
        ...
        if (self.rule in rule_set):
            ...
            rule_set.append(self.rule) #very important here
        output_array.append("%d " % rule_index)
        return line_length + len("%d " % rule_index)

    def print_rule_expansion(self, rule_set, output_array, line_length, rules):
        """docstring for print_rule_expansion"""
        return self.rule.print_rule_expansion(rule_set, output_array, line_length, rules) #diedai
