...
class fuzzyCmeansClustering:
    def __init__(self, q, clusterCenterNumber):
        self.N = int((p - q) / r) + 1
        self.subsequences = data_processing(q, self.N, clusterCenterNumber)
        self.subsequences_hat = [Subsequence(q, clusterCenterNumber) for _ in range(self.N)]
        self.anomaly_score_per_win = [0.0 for _ in range(self.N)]
        self.lam = None
        self.q = q
    ...
    def fuzzyCMeansClustering(self, lam):
        ...
        while True:
            ...
            currentCenterGroup = [Subsequence(self.q, self.clusterCenterNumber) for _ in range(self.clusterCenterNumber)]
